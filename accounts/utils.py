...
def generate_student_id():
    ...
    users = get_user_model().objects.filter(is_student=True)

    max_no = -1

    for user in users:
        match = re.search(rf"{settings.STUDENT_ID_PREFIX}-{year}-(\d+)", user.username)
        if match:
            max_no = max(max_no, int(match.group(1)))

    return f"{settings.STUDENT_ID_PREFIX}-{year}-{max_no + 1}"

def generate_lecturer_id():
    year = datetime.now().strftime("%Y")

    users = get_user_model().objects.filter(is_lecturer=True)

    max_no = -1

    for user in users:
        match = re.search(rf"{settings.LECTURER_ID_PREFIX}-{year}-(\d+)", user.username)
        if match:
            max_no = max(max_no, int(match.group(1)))

    return f"{settings.LECTURER_ID_PREFIX}-{year}-{max_no + 1}"
...
def send_new_account_email(user, password):
    ...
    email = {
        "subject": "Your EduSphere account confirmation and credentials",
